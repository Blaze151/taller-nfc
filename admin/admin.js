...
(function () {
  ...
  async function uploadPendingAttachments(vehicle) {
    for (const service of vehicle.servicios) {
      ...
      for (const stage of ORDEN_ESTADOS_SERVICIO) {
        ...
        const stageLabel = ESTADOS_SERVICIO[stage].label;
  ...
  function renderVehiclePhotoPreview() {
    ...
    if (vFotoPerfilPending) {
      el.innerHTML = `<div class="attach-thumb attach-thumb-remove" style="width:84px;height:84px;"><img src="${URL.createObjectURL(vFotoPerfilPending)}" alt=""><button type="button" id="btn-vphoto-remove" title="Quitar">×</button></div>`;
    } else if (!vFotoPerfilRemoved && currentFotoPerfil) {
      el.innerHTML = `<div class="attach-thumb attach-thumb-remove" style="width:84px;height:84px;"><img src="../${esc(currentFotoPerfil)}" alt=""><button type="button" id="btn-vphoto-remove" title="Quitar">×</button></div>`;
    } else {
      el.innerHTML = `<p style="color:var(--text-faint);font-size:12px;margin:0;">Sin foto todavía.</p>`;
    }
  ...
  function renderHistoryAttachmentsDetail(s) {
    let stagesHtml = "";
    ORDEN_ESTADOS_SERVICIO.forEach((stage) => {
