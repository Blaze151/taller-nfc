...
(function () {
    ...
    container.innerHTML = `
      ...
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);">
        ...
        <div class="grid-2">
          ...
          <div class="form-group"><label class="form-label">${esc(t("saldo_pendiente_label"))}</label><input type="number" min="0" id="as-pago-saldo-pendiente" value="${(efectiveCosto(svcActual) - (Number(svcActual.pago.monto_pagado) || 0)).toFixed(2)}" readonly style="opacity:0.75;"></div>
        </div>` : ""}
  ...
  $("btn-save-and-back").addEventListener("click", async () => {
    const guardadoOk = await saveVehicle();
    if (guardadoOk) showView("dashboard");
  });
