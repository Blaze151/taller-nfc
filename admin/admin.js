...
(function () {
  ...
  function renderVehiclePhotoPreview() {
    ...
    if (vFotoPerfilPending) {
      ...
      el.innerHTML = `<div class="vehicle-photo attach-thumb-remove" style="width:110px;height:110px;"><img src="${esc(rawUrl(currentFotoPerfil))}" alt="" onerror="__imgRetry(this)" onload="__imgLoaded(this)"><button type="button" id="btn-vphoto-remove" title="${esc(t('quitar_btn'))}">×</button></div>`;
    } else {
  ...
  function renderVehicleDocsPreview() {
    ...
    currentDocumentosVehiculo.fotos.forEach((path, i) => {
      fotosHtml += `<div class="attach-thumb attach-thumb-remove"><img src="${esc(rawUrl(path))}" alt="" onerror="__imgRetry(this)" onload="__imgLoaded(this)"><button type="button" data-vdoc-remove-saved-foto="${i}" title="${esc(t('quitar_btn'))}">×</button></div>`;
    });
  ...
  function renderHistoryAttachmentsDetail(s) {
    ...
    ATTACH_STAGES.forEach((stage) => {
      ...
      const fotosHtml = saved.fotos.map((path) => `<div class="attach-thumb"><img src="${esc(rawUrl(path))}" alt="" onerror="__imgRetry(this)" onload="__imgLoaded(this)"></div>`).join("");
    ...
    if (!s.adjuntos && ((s.fotos && s.fotos.length) || (s.documentos && s.documentos.length))) {
      const fotosHtml = (s.fotos || []).map((path) => `<div class="attach-thumb"><img src="${esc(rawUrl(path))}" alt="" onerror="__imgRetry(this)" onload="__imgLoaded(this)"></div>`).join("");
  ...
  function renderStageBlock(stage, infoOverride) {
    ...
    (saved.fotos || []).forEach((path, i) => {
      fotosHtml += `<div class="attach-thumb attach-thumb-remove"><img src="${esc(rawUrl(path))}" alt="" onerror="__imgRetry(this)" onload="__imgLoaded(this)"><button type="button" data-remove-saved-foto="${stage}:${i}" title="${esc(t('quitar_btn'))}">×</button></div>`;
    });
