...
(function () {
  ...
  const STR = {
    es: {
      ...
      msg_conflicto_persistente: "No se pudo guardar: alguien sigue guardando cambios al mismo tiempo que tú. Espera un momento y presiona \"Guardar cambios en GitHub\" de nuevo.",
    ...
    en: {
      ...
      msg_conflicto_persistente: "Couldn't save: someone keeps saving changes at the same time as you. Wait a moment and press \"Save changes to GitHub\" again.",
  ...
  async function collectRecibidoImageDataUrls(vehicle, svcActualForPdf, pendingForPdf) {
    const out = [];
    const saved = (svcActualForPdf.adjuntos && svcActualForPdf.adjuntos.recibido && svcActualForPdf.adjuntos.recibido.fotos) || [];
    ...
    const pending = (pendingForPdf && pendingForPdf.recibido && pendingForPdf.recibido.fotos) || [];
    for (const file of pending) {
      try { out.push(await blobToDataUrl(file)); } catch (err) { /* seguimos */ }
    }
    return out;
  }
  ...
  async function generarInformeRecepcionPDF(vehicle, svcActualForPdf, pendingForPdf) {
    ...
    doc.text("HGR AUTOMOTRIZ", marginX, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(...GREEN);
    doc.text("Informe de Recepción del Vehículo", marginX, y + 6);
    doc.setFontSize(8.5); doc.setTextColor(...GREY);
    doc.text(`Generado: ${new Date().toLocaleString("es-MX")}`, pageW - marginX, y, { align: "right" });
    y += 14;
    ...
    fieldLine("Costo estimado", svcActualForPdf.costo ? `$${Number(svcActualForPdf.costo).toLocaleString("es-MX")}` : "—");
    ...
    const fotos = await collectRecibidoImageDataUrls(vehicle, svcActualForPdf, pendingForPdf);
    if (fotos.length) {
  ...
  // antes que nosotros. En vez de sobrescribir a ciegas, trae lo más reciente de GitHub,
  // vuelve a aplicar SOLO nuestro propio cambio sobre esos datos frescos (con reapplyFn),
  // y reintenta una vez — así no se pierden los cambios que haya hecho la otra sesión.
  ...
  async function saveVehicleWithRetry(vehicle, commitMessage) {
    return saveDatabaseWithRetry(commitMessage, () => { db.vehicles[vehicle.id] = vehicle; });
  }
