...
(function () {
  ...
  const ORDEN_ESTADOS_SERVICIO = ["recibido", "en_proceso", "listo"];
  ...
  function renderDashboard(filter) {
    ...
    list.innerHTML = filtered.map((v) => {
      ...
      <div class="vlist-item" data-id="${v.id}">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end;">${badge}${pagoBadge}</div>
      </div>
    `;
    }).join("");
  ...
  function renderBoard() {
    ...
    const cols = [
      ...
      { key: "sin_activo", label: "Sin servicio activo", icon: "🏁" },
    ];

    board.innerHTML = cols.map((col) => {
      const vehiculos = entries.filter((v) => {
        if (col.key === "sin_activo") return !v.servicio_actual;
        return v.servicio_actual && v.servicio_actual.estado === col.key;
      });
      return `
        <div class="board-col">
          <div class="board-col-title"><span>${col.icon} ${col.label}</span><span class="tag">${vehiculos.length}</span></div>
          ${vehiculos.length ? vehiculos.map((v) => `
            <div class="board-card" data-id="${v.id}">
              <div class="board-card-title">${v.marca || ""} ${v.modelo || ""}</div>
              <div class="board-card-sub">${v.placas || "sin placa"} ${v.cliente_nombre ? "· " + v.cliente_nombre : ""}</div>
              ${v.servicio_actual && v.servicio_actual.pago ? `<div class="board-card-sub">${PAGO_ESTADOS[v.servicio_actual.pago.estado].icon} ${PAGO_ESTADOS[v.servicio_actual.pago.estado].label}</div>` : ""}
              ${v.servicio_actual ? `
                <select data-quick-estado="${v.id}">
                  ${ORDEN_ESTADOS_SERVICIO.map((k) => `<option value="${k}" ${k === col.key ? "selected" : ""}>${ESTADOS_SERVICIO[k].icon} ${ESTADOS_SERVICIO[k].label}</option>`).join("")}
                </select>
                <button class="btn btn-outline btn-sm" style="margin-top:6px;width:100%;" data-quick-finalize="${v.id}">🏁 Finalizar servicio</button>
              ` : `
                <button class="btn btn-outline btn-sm" style="margin-top:6px;width:100%;" data-open-editor="${v.id}">+ Iniciar servicio</button>
              `}
            </div>
          `).join("") : `<p style="color:var(--text-faint);font-size:12px;">Sin vehículos aquí.</p>`}
        </div>
      `;
    }).join("");

    board.querySelectorAll(".board-card-title, .board-card-sub").forEach((el) => {
      el.addEventListener("click", () => openEditor(el.closest(".board-card").dataset.id));
    });
    board.querySelectorAll("[data-open-editor]").forEach((btn) => {
      btn.addEventListener("click", (e) => { e.stopPropagation(); openEditor(btn.dataset.openEditor); });
    });
    board.querySelectorAll("[data-quick-estado]").forEach((select) => {
      select.addEventListener("click", (e) => e.stopPropagation());
      select.addEventListener("change", async (e) => {
        e.stopPropagation();
        await quickUpdateActiveEstado(select.dataset.quickEstado, select.value);
      });
    });
    board.querySelectorAll("[data-quick-finalize]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        e.stopPropagation();
        await quickFinalize(btn.dataset.quickFinalize);
      });
    });
  }
  ...
  function offerWhatsAppListo(datos) {
    if (!confirm(`¿Enviar un mensaje de WhatsApp a ${datos.nombre || "el cliente"} avisando que su vehículo está listo para recoger?`)) return;
    const link = buildWhatsAppListoLink(datos);
    if (!link) {
      showToast("Este vehículo no tiene un teléfono de cliente registrado", "error");
      return;
    }
    window.open(link, "_blank");
  }

  async function quickUpdateActiveEstado(id, nuevoValor) {
    const v = db.vehicles[id];
    if (!v || !v.servicio_actual) return;
    const prevEstado = v.servicio_actual.estado;
    v.servicio_actual.estado = nuevoValor;
    v.servicio_actual.actualizado = todayISO();
    try {
      await saveDatabase(`Actualizar etapa del servicio de ${v.placas || id} a "${ESTADOS_SERVICIO[nuevoValor].label}"`);
      showToast("Etapa actualizada", "success");
      if (nuevoValor === "listo" && prevEstado !== "listo") {
        offerWhatsAppListo({
          nombre: v.cliente_nombre, telefono: v.cliente_telefono,
          marca: v.marca, modelo: v.modelo, placas: v.placas,
          costo: v.servicio_actual.costo,
        });
      }
    } catch (err) {
  ...
  async function quickFinalize(id) {
    const v = db.vehicles[id];
    if (!v || !v.servicio_actual) return;
    if (!confirm(`¿Marcar el servicio de ${v.placas || id} como finalizado? Se moverá al historial.`)) return;
    if (!v.servicios) v.servicios = [];
    v.servicios.unshift(buildHistoricalEntryFromActive(v.servicio_actual, emptyPendingStruct()));
    v.servicio_actual = null;
    try {
      await saveDatabase(`Finalizar servicio de ${v.placas || id}`);
  ...
  function renderStageBlock(stage, infoOverride) {
    ...
    return `
      <div style="margin-top:14px;">
        <div class="card-title">${info.icon} Fotos y documentos — ${info.label}</div>
  ...
  function renderActiveServiceCard() {
    ...
    container.innerHTML = `
      ...
      <div class="actions-row" style="margin-top:16px;">
        ${svcActual.estado === "listo" ? `
          <button class="btn btn-outline" id="btn-not-picked-up">🚗 Aún no lo recoge</button>
          <button class="btn" id="btn-picked-up">✅ Ya lo recogió — Finalizar</button>
        ` : `
  ...
  $("active-service-container").addEventListener("click", (e) => {
    ...
    if (e.target.id === "btn-picked-up") return onFinalizeServiceClick();
    if (e.target.id === "btn-not-picked-up") { showToast("Entendido, sigue como listo para recoger", "success"); return; }
