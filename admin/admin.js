...
(function () {
  ...
  function renderBoard() {
    ...
    cards.push(renderPieCard({
      ...
      segments: [
        { label: "Ya recogido", value: porEtapa.recogido.length, color: GREEN_BRIGHT },
        { label: "Aún no lo recogen", value: porEtapa.listo.length, color: GRIS },
      ],
      holdPct: listoGrupo.length ? Math.round((porEtapa.recogido.length / listoGrupo.length) * 100) : 0,
      ...
      renderVehicleRow: (v) => {
        ...
        return `<div class="vlist-item" data-id="${v.id}" style="margin-bottom:6px;">
          ...
          <span class="tag">${recogido ? "🚗 Recogido" : "⏳ No recogido"}</span>
        </div>`;
      },
    }));
    ...
    cards.push(renderPieCard({
      title: "Pagos", icon: "💳",
      segments: [
        { label: "Pagado", value: pagados.length, color: GREEN },
        { label: "A deber", value: aDeber.length, color: GRIS_CLARO },
        { label: "No pagado", value: noPagados.length, color: GRIS },
      ],
