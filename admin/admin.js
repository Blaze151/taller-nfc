...
(function () {
  ...
  function openEditor(id) {
    ...
    $("f-id").value = v.id || (isNewVehicle ? generarIdSugerido() : "");
  ...
  $("view-editor").addEventListener("input", () => { hasUnsavedChanges = true; });
  $("view-editor").addEventListener("change", () => { hasUnsavedChanges = true; });
  ...
  $("btn-buscar-huerfanos").addEventListener("click", onBuscarHuerfanosClick);

  // Un ID corto o con un patrón simple (como "X1", "AB12") es fácil de
  // adivinar por cualquiera probando la URL pública — eso expondría los
  // datos de ese cliente sin ninguna clave. Este aviso no bloquea nada
  // (el admin puede seguir escribiendo el que quiera), solo avisa.
  function idPareceAdivinable(id) {
    if (!id) return false;
    if (id.length < 6) return true;
    if (/^[A-Za-z]{1,3}\d{1,3}$/.test(id)) return true;
    return false;
  }
  $("f-id").addEventListener("input", (e) => {
    $("id-nfc-warning").style.display = idPareceAdivinable(e.target.value.trim()) ? "block" : "none";
  });
  $("btn-regenerar-id").addEventListener("click", () => {
    $("f-id").value = generarIdSugerido();
    $("id-nfc-warning").style.display = "none";
