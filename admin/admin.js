...
(function () {
  ...
  let lightboxEl, lightboxImg, lightboxCounter, lightboxPhotos = [], lightboxIndex = 0;
  ...
  function emptyOrdenRecepcion() {
    const blank = (list) => Object.fromEntries(list.map(([k]) => [k, null]));
    return {
      enviado: false, aceptado: false, fecha_envio: null, fecha_aceptado: null,
      inventario: { exteriores: blank(INVENTARIO_EXTERIORES), accesorios: blank(INVENTARIO_ACCESORIOS), gasolina: null, danos_notas: "" },
    };
  }
  ...
  const STR = {
    es: {
      ...
      btn_enviar_aceptacion: "📤 Enviar para aceptación",
      cliente_acepto_check: "Cliente aceptó (márcalo al recibir su confirmación)",
      wa_prompt_titulo_orden: "📲 Enviar hoja de recepción",
      msg_orden_wa: "Hola {nombre}, antes de comenzar el servicio de tu {marca} {modelo} (placas {placas}) necesitamos que revises y aceptes la hoja de recepción de tu vehículo (inventario y trabajo solicitado). Revísala aquí: {link} y contéstanos \"Acepto\" cuando estés de acuerdo.",
    ...
    en: {
      ...
      btn_enviar_aceptacion: "📤 Send for acceptance",
      cliente_acepto_check: "Customer accepted (check once they confirm)",
      wa_prompt_titulo_orden: "📲 Send reception checklist",
      msg_orden_wa: "Hi {nombre}, before we start the service on your {marca} {modelo} (plates {placas}) we need you to review and accept your vehicle's reception checklist (inventory and requested work). Check it here: {link} and reply \"I accept\" once you agree.",
  ...
  function buildWhatsAppRecibidoLink({ nombre, telefono, marca, modelo, placas, numero_unidad, id }) {
    let phone = (telefono || "").replace(/\D/g, "");
    if (!phone) return null;
    if (phone.length === 10) phone = "52" + phone;
    const tallerNombre = (db && db.taller && db.taller.nombre) || "el taller";
    ...
    const unidadTxt = numero_unidad ? ` (Unidad ${numero_unidad})` : "";
    const msg = `Hola ${nombre || ""}, tu vehículo ${marca || ""} ${modelo || ""} (placas ${placas || "N/A"})${unidadTxt} fue recibido en ${tallerNombre}. Puedes checar el avance de tu servicio en cualquier momento en este enlace: ${trackUrl}`
      .replace(/[ \t]+/g, " ").trim();
    return `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`;
  }
  ...
  function offerWhatsAppRecibido(datos) {
    const links = [
      { href: buildWhatsAppRecibidoLink({ ...datos, telefono: datos.telefono }), label: "Tel. 1" },
      { href: datos.telefono2 ? buildWhatsAppRecibidoLink({ ...datos, telefono: datos.telefono2 }) : null, label: "Tel. 2" },
    ];
    showWhatsAppPrompt(datos.nombre, links, t("wa_prompt_titulo_recibido"));
  }

  function buildWhatsAppOrdenLink({ nombre, telefono, marca, modelo, placas, id }) {
    let phone = (telefono || "").replace(/\D/g, "");
    if (!phone) return null;
    if (phone.length === 10) phone = "52" + phone;
    const trackUrl = id ? `${TRACK_BASE_URL}?v=${encodeURIComponent(id)}` : TRACK_BASE_URL;
  ...
  function renderInventarioSelect(key, labelKey, value, prefix) {
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px;">
        <label style="font-size:11.5px;color:var(--text-dim);flex:1;">${esc(t(labelKey))}</label>
        <select data-inv="${prefix}:${key}" style="width:118px;">
          <option value="" ${!value ? "selected" : ""}>${esc(t("inv_sin_marcar"))}</option>
          <option value="si" ${value === "si" ? "selected" : ""}>${esc(t("inv_si"))}</option>
          <option value="no" ${value === "no" ? "selected" : ""}>${esc(t("inv_no"))}</option>
        </select>
      </div>`;
  }
  ...
  function renderOrdenRecepcionSection() {
    ...
    const gasOpts = ["vacio", "1/4", "1/2", "3/4", "lleno"];
    const gasKeys = { "vacio": "gasolina_vacio", "1/4": "gasolina_1_4", "1/2": "gasolina_1_2", "3/4": "gasolina_3_4", "lleno": "gasolina_lleno" };
    return `
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);">
        <details class="card-details">
          <summary class="card-title">${esc(t("orden_recepcion_titulo"))}</summary>
          <div class="card-details-body">
        ...
        <div class="grid-2">
          <div>
            ...
            ${INVENTARIO_EXTERIORES.map(([k, lk]) => renderInventarioSelect(k, lk, inv.exteriores[k], "ext")).join("")}
          </div>
          <div>
            <div class="form-label" style="margin-bottom:6px;">${esc(t("inventario_accesorios"))}</div>
            ${INVENTARIO_ACCESORIOS.map(([k, lk]) => renderInventarioSelect(k, lk, inv.accesorios[k], "acc")).join("")}
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">${esc(t("nivel_gasolina_label"))}</label>
          <select id="or-gasolina">
            <option value="" ${!inv.gasolina ? "selected" : ""}>${esc(t("sin_especificar"))}</option>
            ${gasOpts.map((g) => `<option value="${g}" ${inv.gasolina === g ? "selected" : ""}>${esc(t(gasKeys[g]))}</option>`).join("")}
          </select>
        </div>
        <div class="form-group">
        ...
        <div style="font-size:11.5px;color:${estadoColor};margin-bottom:10px;">${estadoTxt}</div>
  ...
  function readActiveServiceFieldsFromDom() {
    ...
    document.querySelectorAll("#active-service-container [data-inv]").forEach((sel) => {
      ...
      target[key] = sel.value || null;
    });
    if ($("or-gasolina")) or.inventario.gasolina = $("or-gasolina").value || null;
  ...
  $("active-service-container").addEventListener("change", (e) => {
    ...
    if (t.id === "or-aceptado") {
      readActiveServiceFieldsFromDom();
      svcActual.orden_recepcion.aceptado = t.checked;
      svcActual.orden_recepcion.fecha_aceptado = t.checked ? todayISO() : null;
      renderActiveServiceCard();
      return;
    }
