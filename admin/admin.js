...
(function () {
  ...
  // (mientras su CDN termina de publicarlo). En vez de dejar el ícono roto,
  // reintentamos unas veces con retraso creciente antes de rendirnos.
  window.__imgRetry = function (el, maxRetries) {
    maxRetries = maxRetries == null ? 4 : maxRetries;
    const n = (Number(el.dataset.retryN) || 0) + 1;
    if (n > maxRetries) {
      const fallback = document.createElement("div");
      fallback.style.cssText = "width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:20px;";
      fallback.textContent = "🚗";
      el.replaceWith(fallback);
      return;
    }
    el.dataset.retryN = String(n);
    const base = el.dataset.baseSrc || el.src.split("?")[0];
    el.dataset.baseSrc = base;
    setTimeout(() => { el.src = base + "?r=" + n + "-" + Date.now(); }, n * 2500);
  };
  ...
  function renderDashboard(filter) {
    ...
    list.innerHTML = filtered.map((v) => {
      ...
      const photoHtml = v.foto_perfil
        ? `<div class="attach-thumb" style="width:50px;height:50px;flex-shrink:0;"><img src="../${esc(v.foto_perfil)}" alt="" onerror="__imgRetry(this)"></div>`
  ...
  function renderVehiclePhotoPreview() {
    ...
    if (vFotoPerfilPending) {
      ...
      el.innerHTML = `<div class="vehicle-photo attach-thumb-remove" style="width:110px;height:110px;"><img src="../${esc(currentFotoPerfil)}" alt="" onerror="__imgRetry(this)"><button type="button" id="btn-vphoto-remove" title="${esc(t('quitar_btn'))}">×</button></div>`;
    } else {
  ...
  function renderVehicleDocsPreview() {
    ...
    currentDocumentosVehiculo.fotos.forEach((path, i) => {
      fotosHtml += `<div class="attach-thumb attach-thumb-remove"><img src="../${esc(path)}" alt="" onerror="__imgRetry(this)"><button type="button" data-vdoc-remove-saved-foto="${i}" title="${esc(t('quitar_btn'))}">×</button></div>`;
    });
  ...
  function renderHistoryAttachmentsDetail(s) {
    ...
    ATTACH_STAGES.forEach((stage) => {
      ...
      const fotosHtml = saved.fotos.map((path) => `<div class="attach-thumb"><img src="../${esc(path)}" alt="" onerror="__imgRetry(this)"></div>`).join("");
    ...
    if (!s.adjuntos && ((s.fotos && s.fotos.length) || (s.documentos && s.documentos.length))) {
      const fotosHtml = (s.fotos || []).map((path) => `<div class="attach-thumb"><img src="../${esc(path)}" alt="" onerror="__imgRetry(this)"></div>`).join("");
  ...
  function renderStageBlock(stage, infoOverride) {
    ...
    (saved.fotos || []).forEach((path, i) => {
      fotosHtml += `<div class="attach-thumb attach-thumb-remove"><img src="../${esc(path)}" alt="" onerror="__imgRetry(this)"><button type="button" data-remove-saved-foto="${stage}:${i}" title="${esc(t('quitar_btn'))}">×</button></div>`;
    });
  ...
  async function saveVehicle() {
    ...
    try {
      ...
      if (hadNewPhoto) showToast(t("msg_foto_puede_tardar"), "", 6500);
