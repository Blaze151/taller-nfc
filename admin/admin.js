...
(function () {
  ...
  const STR = {
    es: {
      ...
      cotiz_observaciones_label: "Observaciones",
      cotiz_observaciones_placeholder: "ej. Tiempo estimado de entrega, el precio incluye refacciones y mano de obra, presupuesto válido por 5 días...",
      ...
      msg_orden_wa: "Hola {nombre}, recibimos tu vehículo {marca} {modelo} (placas {placas}){unidad}. Aquí está el informe de recepción con todos los detalles del servicio: {pdf} — revísalo y confírmanos que estás de acuerdo. Y aquí puedes checar el progreso de tu servicio en cualquier momento: {link}",
    ...
    en: {
      ...
      cotiz_observaciones_label: "Notes",
      cotiz_observaciones_placeholder: "e.g. Estimated delivery time, price includes parts and labor, quote valid for 5 days...",
      ...
      msg_orden_wa: "Hi {nombre}, we received your {marca} {modelo} (plates {placas}){unidad}. Here's the reception report with all the service details: {pdf} — please review it and confirm you agree. And here you can check your service's progress at any time: {link}",
  ...
  async function generarInformeRecepcionPDF(vehicle, svcActualForPdf, pendingForPdf) {
    ...
    const DARK = [20, 23, 26], GREEN = [111, 190, 68], GREY = [110, 110, 110];
    ...
    const logoSize = 20;
    const textX = logoDataUrl ? marginX + logoSize + 5 : marginX;
    if (logoDataUrl) {
      try { doc.addImage(logoDataUrl, "PNG", marginX, y - 6, logoSize, logoSize); } catch (err) { /* seguimos sin logo si falla */ }
    }

    doc.setFont("helvetica", "bold"); doc.setFontSize(16); doc.setTextColor(...DARK);
    doc.text("HGR AUTOMOTRIZ", textX, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(...GREEN);
    doc.text("Informe de Recepción del Vehículo", textX, y + 6);
    doc.setFontSize(8); doc.setTextColor(...GREY);
    doc.text("Calle Cuba #83, Col. Aviación, Torreón, Coahuila.", textX, y + 11.5);
    ...
    if (svcActualForPdf.tiempo_entrega_aprox) fieldLine("Tiempo de entrega aproximado", svcActualForPdf.tiempo_entrega_aprox);
    ...
    doc.text("Documento generado automáticamente por el sistema NFC de HGR Automotriz.", marginX, pageH - 10);
  ...
  async function generarCotizacionPDF(vehicle, svcActualForPdf, pendingForPdf) {
    ...
    doc.text("HGR AUTOMOTRIZ", textX, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(...GREEN);
    doc.text("Presupuesto de Servicio", textX, y + 6);
    doc.setFontSize(8); doc.setTextColor(...GREY);
    doc.text("Calle Cuba #83, Col. Aviación, Torreón, Coahuila.", textX, y + 11.5);
    ...
    if (cot.observaciones) {
      ...
      doc.setFont("helvetica", "normal"); doc.setFontSize(9.5); doc.setTextColor(...DARK);
      const lines = doc.splitTextToSize(cot.observaciones, pageW - marginX * 2);
    ...
    doc.text("Sergio Alejandro Guardado Martínez", pageW / 2, y, { align: "center" });
    y += 4.5;
    doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(...GREY);
    doc.text("HGR Automotriz · Calle Cuba #83, Col. Aviación, Torreón, Coahuila. · Cel: 8711044146", pageW / 2, y, { align: "center" });
    y += 12;

    // Fotos: Recibido y En proceso, separadas con su propio subtítulo
    const fotosRecibido = await collectStageImageDataUrls("recibido", svcActualForPdf, pendingForPdf);
    const fotosEnProceso = await collectStageImageDataUrls("en_proceso", svcActualForPdf, pendingForPdf);
    ...
    await drawPhotoGrid("Fotos del diagnóstico / en proceso", fotosEnProceso);
  ...
  async function runWithConcurrency(items, limit, worker) {
    ...
    async function runner() {
      while (idx < items.length) {
        ...
        if (idx < items.length) await new Promise((r) => setTimeout(r, 350));
      }
    }
  ...
  async function deleteVehicleUploadsFolder(vehicleId) {
    let entries;
    try {
      entries = await githubRequest(`uploads/${encodeURIComponent(vehicleId)}`, { method: "GET" });
    } catch (err) {
      return; // no existe la carpeta (o ya está vacía) — nada que borrar
    }
    ...
    for (const entry of entries) {
      ...
      } catch (err) { /* seguimos con los demás aunque uno falle */ }
    }
  }
  ...
  function servicioRowForBoard(s) {
    const v = s.vehicle;
    const monto = s.costo ? `$${Number(s.costo).toLocaleString("es-MX")}` : "";
    return `<div class="vlist-item" data-id="${v.id}" style="margin-bottom:6px;">
      <div>
  ...
  function renderBoard() {
    ...
    const cobradoEfectivo = mesPagados.filter((s) => s.pago.metodo === "efectivo").reduce((sum, s) => sum + (Number(s.costo) || 0), 0);
    const cobradoTransferencia = mesPagados.filter((s) => s.pago.metodo === "transferencia").reduce((sum, s) => sum + (Number(s.costo) || 0), 0);
    const cobradoSinMetodo = mesPagados.filter((s) => !s.pago.metodo).reduce((sum, s) => sum + (Number(s.costo) || 0), 0);
  ...
  function showWhatsAppPrompt(nombre, links, titulo) {
    ...
    const buttonsHtml = validLinks.length
      ? validLinks.map((l) => `<a class="btn wa-prompt-link" href="${l.href}" target="_blank" rel="noopener">${esc(t("enviar_whatsapp_btn"))}${validLinks.length > 1 ? " · " + esc(l.label) : ""}</a>`).join("")
  ...
  function clearEditorForm() {
    ["f-id", "f-placas", "f-numero-unidad", "f-marca", "f-modelo", "f-anio", "f-version", "f-color", "f-combustible",
     "f-vin", "f-km", "f-cliente-nombre", "f-cliente-tel", "f-cliente-tel-2", "f-aceite-tipo", "f-aceite-cap",
      ...
      <div class="vlist-item" style="cursor:default;display:block;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            ...
            <div class="vlist-item-sub">${s.km ? s.km + " km · " : ""}${s.tecnico || ""}${s.costo ? " · $" + Number(s.costo).toLocaleString("es-MX") : ""} ${chips.join(" ")}</div>
          </div>
    ...
    return `
      <details class="card-details" id="stage-details-${stage}" style="margin-top:14px;" ${wasOpen ? "open" : ""}>
        ...
        <div class="card-details-body">
          ...
          <div>${docsHtml}</div>
          ${stage === "recibido" ? renderOrdenRecepcionSection() : ""}
          ${stage === "en_proceso" ? renderCotizacionSection() : ""}
        </div>
      </details>`;
  }
  ...
  function renderCotizacionSection() {
    ...
    const prevDetails = document.getElementById("cotizacion-details");
    const wasOpen = prevDetails ? prevDetails.open : false;
    const { subtotal, iva, total } = calcCotizacionTotales(cot.items, cot.incluir_iva);
    ...
    return `
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);">
        <details class="card-details" id="cotizacion-details" ${wasOpen ? "open" : ""}>
          <summary class="card-title" style="font-size:12px;">${esc(t("cotizacion_titulo"))}</summary>
          <div class="card-details-body">
  ...
  function renderOrdenRecepcionSection() {
    ...
    const prevDetails = document.getElementById("orden-recepcion-details");
    const wasOpen = prevDetails ? prevDetails.open : false;
    const estadoTxt = or.aceptado
      ? `${esc(t("orden_estado_aceptado"))} ${formatFechaCorta(or.fecha_aceptado)}`
      : or.enviado
    ...
    return `
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);">
        <details class="card-details" id="orden-recepcion-details" ${wasOpen ? "open" : ""}>
          <summary class="card-title" style="font-size:12px;">${esc(t("orden_recepcion_titulo"))}</summary>
          <div class="card-details-body">
  ...
  $("active-service-container").addEventListener("change", (e) => {
    ...
    if (tgt.id === "as-estado") {
      ...
      readActiveServiceFieldsFromDom();
      renderActiveServiceCard();
      if (tgt.value === "listo" && prevEstado !== "listo") {
        offerWhatsAppListo({
    ...
    if (tgt.id === "or-aceptado") {
      ...
      if (tgt.checked && svcActual.estado === "recibido") {
        ...
        svcActual.actualizado = todayISO();
        showToast(t("msg_paso_a_en_proceso"), "success");
      }
      renderActiveServiceCard();
      return;
    }
  ...
  async function deleteVehicle() {
    ...
    try {
      ...
      deleteVehicleUploadsFolder(vehicleIdToClean); // en segundo plano, no bloquea la navegación
      renderDashboard($("search-box").value);
      showView("dashboard");
    } catch (err) {
      if (err.status === 409) {
