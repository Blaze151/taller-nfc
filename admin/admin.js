...
(function () {
  ...
  const ESTADOS = {
    recibido: { label: "Recién llegado", icon: "📥" },
  ...
  function renderDashboard(filter) {
    ...
    list.innerHTML = filtered.map((v) => {
      ...
      const estadoInfo = ESTADOS[estadoValor];
      return `
      <div class="vlist-item" data-id="${v.id}">
        <div>
          <div class="vlist-item-title">${v.marca || ""} ${v.modelo || ""} — ${v.placas || "sin placa"}</div>
          <div class="vlist-item-sub">ID: ${v.id} ${v.cliente_nombre ? "· " + v.cliente_nombre : ""} · ${(v.servicios || []).length} servicios</div>
        </div>
        <span class="status-chip st-${estadoValor}">${estadoInfo.icon} ${estadoInfo.label}</span>
      </div>
    `;
    }).join("");
  ...
  function renderBoard() {
    ...
    board.innerHTML = ORDEN_ESTADOS.map((key) => {
      const info = ESTADOS[key];
      const vehiculos = entries.filter((v) => ((v.estado && v.estado.valor) || "recibido") === key);
      return `
        <div class="board-col">
          <div class="board-col-title"><span>${info.icon} ${info.label}</span><span class="tag">${vehiculos.length}</span></div>
          ${vehiculos.length ? vehiculos.map((v) => `
            <div class="board-card" data-id="${v.id}">
              ...
              <select data-quick-estado="${v.id}">
                ${ORDEN_ESTADOS.map((k) => `<option value="${k}" ${k === key ? "selected" : ""}>${ESTADOS[k].icon} ${ESTADOS[k].label}</option>`).join("")}
              </select>
            </div>
    ...
    board.querySelectorAll("[data-quick-estado]").forEach((select) => {
      ...
      select.addEventListener("change", async (e) => {
        e.stopPropagation();
        await quickUpdateEstado(select.dataset.quickEstado, select.value);
      });
    });
  }

  async function quickUpdateEstado(id, nuevoValor) {
    const v = db.vehicles[id];
    if (!v) return;
    v.estado = { valor: nuevoValor, actualizado: new Date().toISOString().slice(0, 10) };
    try {
      await saveDatabase(`Actualizar estado de ${v.placas || id} a "${ESTADOS[nuevoValor].label}"`);
  ...
  function openEditor(id) {
    ...
    $("f-estado").value = (v.estado && v.estado.valor) || "recibido";
  ...
  function buildVehicleFromForm() {
    ...
    const estadoPrevio = isNewVehicle ? null : (db.vehicles[currentVehicleId] && db.vehicles[currentVehicleId].estado);
    const estadoObj = (estadoPrevio && estadoPrevio.valor === nuevoEstado)
      ? estadoPrevio
      : { valor: nuevoEstado, actualizado: new Date().toISOString().slice(0, 10) };
  ...
  (async function init() {
    // Precarga usuario/repo/rama por defecto para no tener que escribirlos cada vez
