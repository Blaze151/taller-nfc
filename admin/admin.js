...
(function () {
  ...
  // Sube adjuntos pendientes del historial (por compatibilidad; el historial normalmente
  // ya llega con todo subido, salvo el servicio recién finalizado en esta misma sesión).
  ...
  function buildHistoricalEntryFromActive(active, pending) {
    ...
    return {
      ...
      fotos: ORDEN_ESTADOS_SERVICIO.flatMap((s) => (active.adjuntos[s] && active.adjuntos[s].fotos) || []),
      documentos: ORDEN_ESTADOS_SERVICIO.flatMap((s) => (active.adjuntos[s] && active.adjuntos[s].documentos) || []),
      _pendingFotos: ORDEN_ESTADOS_SERVICIO.flatMap((s) => (pending[s] && pending[s].fotos) || []),
      _pendingDocumentos: ORDEN_ESTADOS_SERVICIO.flatMap((s) => (pending[s] && pending[s].documentos) || []),
    };
  }
