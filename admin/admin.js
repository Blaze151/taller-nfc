...
(function () {
  ...
  const STR = {
    es: {
      ...
      msg_informe_trabajo_wa: "Hola {nombre}, tu {marca} {modelo} (placas {placas}){unidad} ya está listo. Aquí está el informe completo de lo que se hizo: {pdf}. ¡Gracias por tu confianza en HGR Automotriz!",
    ...
    en: {
      ...
      msg_informe_trabajo_wa: "Hi {nombre}, your {marca} {modelo} (plates {placas}){unidad} is ready. Here's the full report of what was done: {pdf}. Thank you for trusting HGR Automotriz!",
  ...
  async function generarInformeTrabajoPDF(vehicle, svcActualForPdf, pendingForPdf) {
    ...
    doc.text("HGR AUTOMOTRIZ", textX, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(...GREEN);
    doc.text("Informe de Trabajo", textX, y + 6);
    doc.setFontSize(8); doc.setTextColor(...GREY);
    doc.text("Calle Cuba #83, Col. Aviación, Torreón, Coahuila.", textX, y + 11.5);
    ...
    doc.text("¡Gracias por tu confianza en HGR Automotriz!", pageW / 2, y, { align: "center" });
  ...
  $("active-service-container").addEventListener("change", (e) => {
    ...
    if (tgt.id === "as-estado") {
      ...
      if (tgt.value === "listo" && prevEstado !== "listo") {
        // Si hay una cotización con conceptos cargados, el monto a cobrar es
        // su total (con IVA incluido si aplica) — no el campo "Precio" suelto,
        // que se queda desactualizado en cuanto se usa la cotización.
        const cot = svcActual.cotizacion;
        const tieneCotizacion = cot && (cot.items || []).some((it) => (Number(it.cantidad) || 0) * (Number(it.costo_unitario) || 0) > 0);
        const montoAviso = tieneCotizacion ? calcCotizacionTotales(cot.items, cot.incluir_iva).total : svcActual.costo;
        saveVehicle();
        offerWhatsAppListo({
  ...
  async function saveVehicle() {
    ...
    skippedPhotoNames = [];
    try {
      // Estas 3 tocan partes distintas del vehículo (historial, servicio activo,
      // y nivel-vehículo) y no dependen entre sí — se corren en paralelo para
      // acelerar el guardado cuando hay fotos pendientes en más de una.
      await Promise.all([
        uploadPendingAttachments(vehicle),
        uploadPendingActiveServiceAttachments(vehicle),
        uploadVehicleLevelAttachments(vehicle),
      ]);
