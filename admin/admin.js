...
(function () {
  ...
  function guardarBorradorLocal() {
    if (!currentVehicleId || !hasUnsavedChanges) return;
    let vehicle;
    try {
    ...
    try {
      ...
      localStorage.setItem("draft_" + currentVehicleId, JSON.stringify({ vehicle: JSON.parse(json), savedAt: new Date().toISOString() }));
  ...
  function borrarBorradorLocal(id) {
    try { localStorage.removeItem("draft_" + id); } catch (err) {}
  }

  function updateUnsavedIndicator() {
    const el = $("unsaved-indicator");
    if (el) el.style.display = hasUnsavedChanges ? "block" : "none";
  }

  function setEditorControlsBusy(busy) {
    [
      "as-estado", "btn-finalize-service", "btn-cancel-service", "btn-back",
      "btn-save-visible", "btn-enviar-orden", "btn-enviar-cotizacion", "btn-enviar-informe-trabajo",
    ].forEach((id) => {
      const el = $(id);
  ...
  function tallerInfoForPdf() {
    ...
    return {
      nombre: t2.nombre || "HGR Automotriz",
      direccion: t2.direccion || "",
      telefono: t2.telefono || "",
      firmante: t2.firmante || "Sergio Alejandro Guardado Martínez",
      agradecimiento: t2.mensaje_agradecimiento || `¡Gracias por tu confianza en ${t2.nombre || "HGR Automotriz"}!`,
    };
  }
  ...
  async function generarInformeRecepcionPDF(vehicle, svcActualForPdf, pendingForPdf) {
    ...
    doc.text(taller.direccion, textX, y + 11.5);
  ...
  function openEditor(id) {
    ...
    // Si hay un borrador local de este vehículo (algo que quedó sin
    // guardar), se ofrece recuperarlo antes de cargar la versión de GitHub.
    if (id) {
      ...
      if (draft && draft.vehicle) {
        ...
        if (confirm(t("confirm_recuperar_borrador").replace("{fecha}", fechaBorrador))) {
          db.vehicles[id] = draft.vehicle;
        }
    ...
    container.innerHTML = `
      ...
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);">
        ...
        <div class="grid-2">
          ...
          <div class="form-group"><label class="form-label">${esc(t("saldo_pendiente_label"))}</label><input type="number" min="0" id="as-pago-saldo-pendiente" value="${svcActual.pago.saldo_pendiente ?? ""}"></div>
        </div>` : ""}
  ...
  function readActiveServiceFieldsFromDom() {
    ...
    if ($("as-pago-saldo-pendiente")) svcActual.pago.saldo_pendiente = $("as-pago-saldo-pendiente").value ? Math.max(0, Number($("as-pago-saldo-pendiente").value)) : null;
  ...
  async function saveVehicle() {
    ...
    try {
      ...
      hasUnsavedChanges = false;
      updateUnsavedIndicator();
      borrarBorradorLocal(vehicle.id);
  ...
  $("view-editor").addEventListener("input", () => { hasUnsavedChanges = true; updateUnsavedIndicator(); });
  $("view-editor").addEventListener("change", () => { hasUnsavedChanges = true; updateUnsavedIndicator(); });
  ...
  $("view-editor").addEventListener("click", (e) => {
    if (e.target.closest("button") || e.target.closest(".fuel-track")) { hasUnsavedChanges = true; updateUnsavedIndicator(); }
  });
  ...
  draftIntervalId = setInterval(guardarBorradorLocal, 12000);

  $("search-box").addEventListener("input", (e) => renderDashboard(e.target.value));
  $("filter-select").addEventListener("change", () => renderDashboard($("search-box").value));
  $("sort-select").addEventListener("change", () => renderDashboard($("search-box").value));
