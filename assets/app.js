...
(function () {
  ...
  function renderAttachments(s) {
    ...
    let html = "";
    if (fotos.length) {
      html += `<div class="attach-gallery">` + fotos.map((path) => `
        <a class="attach-thumb" href="${esc(path)}" target="_blank" rel="noopener">
    ...
    if (servicios.length) {
      timelineHtml = `<ul class="timeline">` + servicios.map((s, i) => `
        <li class="timeline-item ${i === 0 ? "latest" : ""}">
          ...
          <div class="timeline-meta">${formatKm(s.km)}${s.tecnico ? " · " + esc(s.tecnico) : ""}</div>
          ${renderAttachments(s)}
        </li>`).join("") + `</ul>`;
    } else {
    ...
    contentEl.innerHTML = `
      <div class="ticket">
        <div class="ticket-head">
          ...
          <span class="status-stamp">Verificado</span>
        </div>
        <div class="grid-2">
          <div class="field">
      ...
      <div class="card">
        ...
        ${ultimo ? `
          ...
          <div class="timeline-meta">${formatFecha(ultimo.fecha)} · ${formatKm(ultimo.km)}${ultimo.tecnico ? " · " + esc(ultimo.tecnico) : ""}</div>
