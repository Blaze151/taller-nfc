...
(function () {
  ...
  function renderStageSection(activo, stage) {
    ...
    return `
      <div style="margin-top:14px;">
        <div class="card-title" style="margin-bottom:6px;">${info.icon} ${esc(info.label)}</div>
        ${attachHtml}
      </div>`;
  }
  ...
  function renderServiceAttachments(service) {
    if (service.adjuntos) {
      return ORDEN_ESTADOS_SERVICIO.map((stage) => renderStageSection(service, stage)).join("");
    }
  ...
  function renderActiveServiceCard(activo) {
    ...
    return `
      <div class="card" style="border-color:var(--accent);">
        ...
        ${ORDEN_ESTADOS_SERVICIO.map((stage) => renderStageSection(activo, stage)).join("")}
      </div>`;
  }
