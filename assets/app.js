...
(function () {
  ...
  window.__imgRetry = function (el, maxRetries) {
    ...
    if (n > maxRetries) {
      ...
      fallback.className = "vehicle-photo-placeholder";
    ...
    if (fotos.length) {
      ...
      html += `<div class="attach-gallery">` + fotos.map((path, i) => `
        <div class="attach-thumb" data-lightbox-group="${group}" data-lightbox-index="${i}">
          <img src="${esc(path)}" alt="Foto del servicio" loading="lazy" onerror="__imgRetry(this)">
        </div>
      `).join("") + `</div>`;
    }
    if (docs.length) {
  ...
  lightboxImg.addEventListener("error", () => {
    const n = (Number(lightboxImg.dataset.retryN) || 0) + 1;
    if (n > 4) return;
    lightboxImg.dataset.retryN = String(n);
    const base = lightboxImg.dataset.baseSrc || lightboxImg.src.split("?")[0];
    lightboxImg.dataset.baseSrc = base;
    setTimeout(() => { lightboxImg.src = base + "?r=" + n + "-" + Date.now(); }, n * 2500);
  });
          ...
          <div style="display:flex;gap:14px;align-items:flex-start;">
            ${v.foto_perfil ? `
              <div class="vehicle-photo" data-lightbox-group="${JSON.stringify([v.foto_perfil]).replace(/"/g, "&quot;")}" data-lightbox-index="0">
                <img src="${esc(v.foto_perfil)}" alt="Foto del vehículo" onerror="__imgRetry(this)">
              </div>` : `
